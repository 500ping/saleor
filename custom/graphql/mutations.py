...
class CustomCreate(ModelMutation):
    ...
    custom = graphene.Field(CustomType, description="custom return")
    ...
    @classmethod
    def perform_mutation(cls, _root, info, **data):
        custom_instance = models.Custom(
            name=data.get('input').name,
            attribute=data.get('input').attribute,
            description=data.get('input').description,
            pick_date=data.get('input').pick_date,
            new_feature=data.get('input').new_feature,

        )
        custom_instance.save()
        return cls.success_response(custom_instance)
